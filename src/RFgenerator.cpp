...
// RF driver with both USB and eternet interfaces. The system has the following features:
// Relay details:
//  Gen2 has 4 relays
//    K1,K2 on RF+ coil's side
//    K3,K4 on RF- coil's side
//  1.) LED indicators:
//      - RED    = RF on
//      - Yellow = status
//                 flash when tuning
...
//    Version 1.4, Apr 24, 2025
//      - Added trigger gate option to gen 1 RF generator
//    Version 1.5, May 20, 2025
//      - Added support for PWL ramp
//      - Fixed bug in LOADCAL, it was saving!
//    Version 1.6, July 18, 2025
//      - Added quench logic and compile mode
//      - Added compile switch for trigger inverter
...
void (*myAnalogWriteFunction)(int) = myAnalogWrite;
...
int   rampLoops,rampLoops2;
float rampStepSize,rampStepSize2;
...
void Control(void)
{
  if((rfgendata.Mode) && (rfgendata.Enable))
  {
    ...
    #ifdef SINGLEENDED
    float error = rfgendata.Setpoint - RFvoltageP;
    #else
...
#ifdef RFGEN2
// Ramp with a user defined time point during the ramp to change the ramp
// rate by a multiplier.
// The overall ramp time is defined by the user
void prepairRamp(void)
{
  ...
  rampLoops2 = rampStepSize2 = 0;
  if((rfgendata.changePoint == 0) || (rfgendata.changePoint >= rfgendata.rampTime))
  {
    // Calculate loop steps and step size
    rampLoops = rfgendata.rampTime / RAMPLOOPTIME;
    rampStepSize = (rfgendata.rampEndV - rfgendata.rampStartV) / (float)rampLoops;
    if(rfgendata.Enable) setDrive(rfgendata.Drive);
    return;
  }
  //serial->println(rfgendata.rampStartV);
  //serial->println(rfgendata.rampEndV);
  //serial->println(rfgendata.rampMult);
  //serial->println(rfgendata.rampTime);
  //serial->println(rfgendata.changePoint);
  float r1 = (rfgendata.rampStartV - rfgendata.rampEndV)/((rfgendata.rampMult*(rfgendata.rampTime - rfgendata.changePoint)) + rfgendata.changePoint);
  float r2 = r1 * rfgendata.rampMult;
  float c = rfgendata.rampStartV - rfgendata.changePoint * r1;
  //serial->println(r2,4);
  //serial->println(c,4);
  rampLoops = rfgendata.changePoint / RAMPLOOPTIME;
  rampStepSize = (c - rfgendata.rampStartV) / (float)rampLoops;
  rampLoops2 = (rfgendata.rampTime - rfgendata.changePoint) / RAMPLOOPTIME;
  rampStepSize2 = (rfgendata.rampEndV - c) / (float)rampLoops2;
  if(rfgendata.Enable) setDrive(rfgendata.Drive);
}

void prepairRamp2(void)
{
  // Set drive level to support higest ramp level
  float maxV = rfgendata.rampStartV;
  if(rfgendata.rampEndV > maxV) maxV = rfgendata.rampEndV;
...
void executeRamp(void)
{
  ...
  if(rampLoops2<=0) return;
  for(int i=0;i<rampLoops2;i++)
  {
    currentBias += rampStepSize2;
    writeBIAS;
  }
  interrupts();
}
...
void setRampTime(char *val)   { if(valueUpdate(&rfgendata.rampTime, val, 0.1, 10)) SendACK;}
void setRampChangePoint(char *val) { if(valueUpdate(&rfgendata.changePoint, val, 0.1, 10)) SendACK;}
void setRampMult(char *val) { if(valueUpdate(&rfgendata.rampMult, val, 0, 10)) SendACK;}
...
void setTrigger(char *val) 
{
  ...
  else if(token == "GATE") 
  {
    ...
    #ifdef RFGEN2
    attachInterrupt(digitalPinToInterrupt(TRIG), trigRamp_isr, CHANGE);
    #else
    attachInterrupt(digitalPinToInterrupt(TRIG), trigFast_isr, CHANGE);
    #endif
    #ifdef QUENCH
    attachInterrupt(digitalPinToInterrupt(TRIG), trigFast_isr, CHANGE);
    #endif
...
void setTriggerLevel(char *val) 
{
  String token = val;
  #ifdef TRIGINVERTED
  if(token == "HIGH")     rfgendata.triggerLevel = ACTIVE_LOW;
  else if(token == "LOW") rfgendata.triggerLevel = ACTIVE_HIGH;
  else BADARG;
  #else
...
void getTriggerLevel(void) 
{
  SendACKonly;
  #ifdef TRIGINVERTED
  if(rfgendata.triggerLevel == ACTIVE_HIGH) serial->println("LOW");
  if(rfgendata.triggerLevel == ACTIVE_LOW)  serial->println("HIGH");
  #else
...
void useDMA(void)
{
  if(myAnalogWriteFunction != myAnalogWriteDMA)
  {
